...
## Batch 6 — Accessibility, errors/fallback, metadata, deep-link fallback (COMPLETE)

### §12 Production deep-link fallback

- `vite.config.ts` `spaFallback()` plugin (`closeBundle`) copies `dist/index.html` → `dist/404.html`.
  Verified: `dist/404.html` == `dist/index.html`; `dist/Resume.pdf` and `dist/og-image.png` survive
  the build untouched. GitHub Pages Actions deploy uploads all of `dist/`, so no workflow change.
  Real files are served directly; `404.html` only fires on genuine misses → static assets and
  independent entry points are preserved by construction.

### §13 Metadata / social previews

- `index.html`: `<title>` = "Azeem Ehtisham — Software & Systems Engineer"; meta description;
  `theme-color`; canonical; full Open Graph (`type`, `site_name`, `url`, `title`, `description`,
  `image` + dimensions) and Twitter (`summary_large_image`) tags. URLs use `https://azeemme.com`
  (from `public/CNAME`).
- `public/og-image.png` — 1200×630 branded **placeholder** (gradient matching the desktop bg,
  generated by a one-off script that was then removed). Stage 2 replaces it with a designed image.
- `src/routing/DocumentTitle.tsx` — sets `document.title` per route (project pages get
  "<Project> — Azeem Ehtisham"; 404 / project-not-found / terminal / projects list each distinct).
  No SSR / prerendering.

### §11 Error & fallback behaviour

- `src/components/common/RootErrorBoundary.tsx` — full-page non-crashing fallback (Reload / Portfolio
  actions) wrapping the whole app in `App.tsx`. Covers a failed lazy chunk load or render throw.
- `AppErrorBoundary` (Batch 2) still isolates individual desktop windows.
- Root `<Suspense>` fallback is a dark full-bleed panel (never blank white).
- Unknown routes → `NotFound`; unknown project slugs → in-Portfolio project-not-found (Batch 2).
- Media are placeholder `<div>`s (no `<img>` that can 404 in Stage 1); real-media `onError`
  fallback is a Stage 2 concern and noted there.

### §10 Accessibility built into the foundation

- Semantic landmarks: `<nav aria-label>` (dock), `<main id="main" aria-label="Portfolio">`,
  `<main>` on standalone pages. Headings: single `<h1>` per view, ordered `<h2>`/`<h3>`.
- Real `<button>` / `<a>` throughout; dock + title-bar controls keyboard-operable (pre-existing,
  preserved). Card = `<button>` with descriptive `aria-label`. External links carry a visible ↗
  plus `sr-only` "(opens in a new tab)".
- Focus management: project detail focuses its container on mount (`preventScroll`); Escape closes
  it — listener **scoped to the detail element** so it can't collide with the AI modal / Terminal.
- AI modal: `role="dialog"` + `aria-modal`, focus moves to the close button on open and returns to
  the AI dock button on close, Escape closes (capture phase), minimal Tab containment.
- Global `:focus-visible` outline; `.skip-link` (shown on focus) on the mobile Portfolio.
- `.sr-only` utility. `reset.css` keeps document scroll available for standalone / mobile.

### Validation

- npm test: PASS (53/53). npm run lint: PASS. npm run build: PASS (no chunk warning).
- Independent accessibility + regression review: PENDING (subagents) — see Batch 7.

### Manual verification (added)

- **NEEDS MANUAL VERIFICATION:** social preview actually renders (title/description/image) when the
  URL is unfurled — the og:image is a branded placeholder, not a designed card.
- **NEEDS MANUAL VERIFICATION:** production deep-link refresh on GitHub Pages (`/projects/suits`
  reload) resolves via `404.html`; `azeemme.com/Resume.pdf` still serves the PDF.
- **NEEDS MANUAL VERIFICATION:** visible focus outlines / skip link / modal focus trap in a real
  browser + screen reader.
- **NEEDS MANUAL VERIFICATION:** `document.title` updates on client navigation.
...
- [x] Batch 6 - Accessibility, errors, metadata, deployment
...
- Batch 4: `908da37`
- Batch 5: `ccc47f2`
- Batch 6: pending checkpoint commit
